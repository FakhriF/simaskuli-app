              ...
              <Link key={thread.id} href={`/forum/${thread.id}`}>
                <div className="block p-6 border border-black rounded-md shadow-md hover:bg-gray-100 flex items-end justify-between">
                  <div className="flex-grow">
                    <h5 className="mb-2 text-xl font-bold">{thread.title}</h5>
                    <p className="font-normal text-gray-6000">By {thread.user_id}, {new Date(thread.created_at).toISOString().split('T')[0]}</p>
                  </div>
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-400">{thread.replies} replies</span>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="w-4 h-4">
                      <path d="M8 9.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z" />
                      <path fillRule="evenodd" d="M1.38 8.28a.87.87 0 0 1 0-.566 7.003 7.003 0 0 1 13.238.006.87.87 0 0 1 0 .566A7.003 7.003 0 0 1 1.379 8.28ZM11 8a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" clipRule="evenodd" />
                    </svg>
                    <span className="text-sm text-gray-400">{thread.views} views</span>
                  </div>
                </div>
              </Link>
            ))}
